...
int main()
{
    ...
    */
    ...
    std::ofstream csv_file("memory_bandwidth_unrolling_and_blocked.csv");
        ...
        for (size_t iteration = 0; iteration < 10000 * (dimension_sizes[29] / array_size); ++iteration)
        {
            for (size_t i_block = 0; i_block < nBlocks; i_block++)
            {
                for (size_t j_block = 0; j_block < nBlocks; j_block++)
                {
                    ...
                    {
                        for (size_t j = j_block * blockSize; j < (j_block + 1) * blockSize; j = j + 5)
                        {
                            array_B[i * array_size + j] = array_A[j * array_size + i];
                            array_B[(i + 1) * array_size + j] = array_A[j * array_size + i + 1];
                            array_B[(i + 2) * array_size + j] = array_A[j * array_size + i + 2];
                            array_B[(i + 3) * array_size + j] = array_A[j * array_size + i + 3];
                            array_B[(i + 4) * array_size + j] = array_A[j * array_size + i + 4];
                        }
                    }
                }
            }
